...
class Twig_Tests_TemplateTest extends \PHPUnit\Framework\TestCase
{
     ...
     * @expectedExceptionMessage Block "unknown" on template "users_index.twig" does not exist in "users_index.twig".
     */
    public function testRenderBlockWithUndefinedBlock()
    {
        $twig = new Twig_Environment($this->getMockBuilder('Twig_LoaderInterface')->getMock());
        $template = new Twig_TemplateTest($twig, 'users_index.twig');
        try {
    ...
    /**
     * @expectedException Twig_Error_Runtime
     * @expectedExceptionMessage Block "foo" should not call parent() in "users_index.twig" as the block does not exist in the parent template "parent.twig"
     */
    public function testDisplayBlockWithUndefinedParentBlock()
    {
        $twig = new Twig_Environment($this->getMockBuilder('Twig_LoaderInterface')->getMock());
        $template = new Twig_TemplateTest($twig, 'parent.twig');
        $template->displayBlock('foo', array(), array('foo' => array(new Twig_TemplateTest($twig, 'users_index.twig'), 'block_foo')), false);
    }
    ...
    public function getGetAttributeTests()
    {
        ...
        $tests = array_merge($tests, array(
            array(false, null, 42, 'a', array(), $anyType, 'Impossible to access an attribute ("a") on a integer variable ("42") in "users_index.twig".'),
            array(false, null, 'string', 'a', array(), $anyType, 'Impossible to access an attribute ("a") on a string variable ("string") in "users_index.twig".'),
            array(false, null, array(), 'a', array(), $anyType, 'Key "a" does not exist as the array is empty in "users_index.twig".'),
        ));
...
class Twig_TemplateTest extends Twig_Template
{
    ...
    public function __construct(Twig_Environment $env, $name = 'users_index.twig')
    {
